...
int main( int argc, char** argv )
{
  ...
  cv::findContours( img_bin.clone(), contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE );
  ...
  for ( int i = 0; i < contours.size(); i++ )
  {
    ...
    if ( std::fabs( contourArea(contours[i]) ) < 100 || !isContourConvex(approx) ) continue;

    if ( approx.size() == 3 ) setLabel( img_dest, "TRI", contours[i] ); // Triangles
